...
// These methods work only passing instance of req.file containing buffer contents
 ...
 * @return {Promise<void>}
 */
export const streamBufferToLFS = async (data: Buffer): Promise<void> => {
 ...
 * @param buffer
 */
export const streamBufferToGCS = async (buffer: Buffer) => {
  ...
  return new Promise((resolve, reject) => {
    const blob = bucket.file('ems-service-folder/dtl/test.xml');
...
export const streamFileToGCS = async (
  ...
  return new Promise((resolve, reject) => {
    const blob = bucket.file(
      `ems-service-folder/${directory}/${slugify(fileName || uuid)}`,
    );
 ...
 * @param blobName
 */
export const deleteFileFromGCS = async (blobName: string) => {
  try {
