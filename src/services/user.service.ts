...
import { ISortingOptions } from '../interfaces/common.interface';
import {
  CurrentUserPayload,
  IUserQueryFilter,
...
export default {
   ...
   */
  createUser: async (body: IUserDocument, file: Express.Multer.File) => {
   ...
   * @param options
   * @return {Promise<*>}   */
  findUsers: async (filter: IUserQueryFilter, options: ISortingOptions) => {
    ...
    // Clean empty values
    const cleanedFilters = cleanDeep(reqQuery);
    ...
    const users: Array<IUserDocument> = await User.find(JSON.parse(queryStr))
      .sort(sorting || sortBy)
  ...
  findUserAndUpdate: async (
    ...
    if (file) {
      ...
      const cleanedObj = cleanDeep(pictureObject);
    ...
    // 3. Cleaning undefined values and create a cleaned object
    const cleanedObject = cleanDeep(mergedObject);
   ...
   * @return {Promise<*>}
   */
  findUserAndUpdatePassword: async (
    userId: string,
    body: IUserDocument,
    currentUser: CurrentUserPayload,
  ) => {
